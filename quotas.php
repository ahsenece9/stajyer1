...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'period_delete') {
        $id = (int) ($_POST['id'] ?? 0);
        $pRow = db()->prepare('SELECT name FROM internship_periods WHERE id = ?');
        $pRow->execute([$id]);
        $pName = (string) ($pRow->fetchColumn() ?: ('ID: ' . $id));
        db()->prepare('DELETE FROM internship_periods WHERE id = ?')->execute([$id]);
        log_action('donem_sil', 'Dönem silindi: ' . $pName);
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <div style="display:flex; align-items:center; width:100%;">
                                ...
                                <a href="period_quotas.php?period_id=<?= (int)$p['id'] ?>" style="color: var(--primary); font-weight: 700; font-size: 13px; text-decoration: underline; text-underline-offset: 3px; display: inline-flex; align-items: center; gap: 4px;">
                                    <span class="ms" style="font-size:16px;">list_alt</span> Kontenjanlar
                                </a>
                                <span style="flex:1; display:flex; justify-content:flex-end; padding-right:12px;">
