...
// Oturum hatalarını önlemek için yerel dizin ayarı
...
{
    return defined('APP_MODE') && APP_MODE === 'demo' ? 'demo' : 'live';
}
...
function db(): PDO
{
    static $pdo = null;
    if ($pdo === null) {
        // Moda göre veritabanı adını seç (demo veya canlı)
        $dbName = (app_mode() === 'demo' && defined('DB_NAME_DEMO')) ? DB_NAME_DEMO : DB_NAME;
        $pdo = new PDO(
        ...
        // Kayıt onay durumu: pending (onay bekliyor) / active (aktif) / rejected (reddedildi)
        try {
            $pdo->query("SELECT status FROM users LIMIT 1");
        } catch (PDOException $e) {
            try {
                $pdo->exec("ALTER TABLE users ADD COLUMN status ENUM('pending','active','rejected') NOT NULL DEFAULT 'active' AFTER role");
            } catch (PDOException $ex) {}
        }
...
function svg_icon(string $name): string
{
    static $paths = [
        ...
        'corporate_fare' => '<rect x="4" y="2" width="16" height="20" rx="2" ry="2"/><path d="M9 22v-4h6v4"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M12 6h.01"/><path d="M12 10h.01"/><path d="M12 14h.01"/><path d="M16 10h.01"/><path d="M16 14h.01"/><path d="M8 10h.01"/><path d="M8 14h.01"/>'
    ];
 ...
 */
function log_action(string $action, string $detail = '', ?string $actorName = null): void
{
    // Oturum açıksa oturumdaki kullanıcı; değilse (halka açık işlem) verilen aktör adı ya da "Ziyaretçi".
    $userId   = $_SESSION['user_id'] ?? null;
    $userName = $_SESSION['user_name'] ?? ($actorName !== null && $actorName !== '' ? $actorName : 'Ziyaretçi');
    try {
...
const LOG_ACTIONS = [
    ...
    'cikis'               => ['Çıkış Yapıldı', 'info'],
    'giris_hatali'        => ['Hatalı Giriş Denemesi', 'devamsiz'],
    'giris_reddedildi'    => ['Onaysız Giriş Denemesi', 'devamsiz'],
    // Kayıt onay akışı
    'kayit_talebi'        => ['Yeni Kayıt Talebi', 'info'],
    'kayit_onayla'        => ['Kayıt Onaylandı', 'aktif'],
    'kayit_reddet'        => ['Kayıt Reddedildi', 'baslamadi'],
    // Stajyer işlemleri
    ...
    'yoklama_giris'       => ['QR ile Giriş Yoklaması', 'info'],
    'yoklama_cikis'       => ['QR ile Çıkış Yoklaması', 'info'],
    'not_ekle'            => ['Staj Notu Eklendi', 'info'],
    'not_guncelle'        => ['Staj Notu Güncellendi', 'info'],
    ...
    // Başvuru modülü
    'basvuru'             => ['Staj Başvurusu Yapıldı', 'info'],
    'basvuru_onay'        => ['Başvuru Onaylandı', 'aktif'],
    'basvuru_red'         => ['Başvuru Reddedildi', 'baslamadi'],
    // Dönem & kontenjan
    'donem_ekle'          => ['Dönem Eklendi', 'info'],
    'donem_sil'           => ['Dönem Silindi', 'baslamadi'],
    'kontenjan_ekle'      => ['Kontenjan Eklendi', 'info'],
    'kontenjan_guncelle'  => ['Kontenjan Güncellendi', 'info'],
    'kontenjan_sil'       => ['Kontenjan Silindi', 'baslamadi'],
    // Kullanıcı yönetimi
    'kullanici_ekle'      => ['Kullanıcı Eklendi', 'aktif'],
    'kullanici_guncelle'  => ['Kullanıcı Güncellendi', 'info'],
    'kullanici_rol_guncelle' => ['Kullanıcı Rolü Güncellendi', 'info'],
    'kullanici_sil'       => ['Kullanıcı Silindi', 'baslamadi'],
    'profil_guncelle'     => ['Profil Güncellendi', 'info'],
...
function render_head(string $title): void
{
    ...
    echo '<script>';
    echo '  (function() {';
    echo '    var theme = localStorage.getItem("theme") || "corporate";';
    echo '    localStorage.setItem("theme", theme);';
    echo '    var bgColors = {"dark": "#091324", "light": "#f7f9fb", "ocean": "#0f0d36", "rose": "#fff0f5", "corporate": "#f4f6fa"};';
    echo '    document.documentElement.style.backgroundColor = bgColors[theme] || "#f4f6fa";';
    ...
    echo '      else if (theme === "corporate") overlay = "rgba(13, 25, 52, 0.40)";';
    ...
    // SweetAlert2 — bildirim popup'ları için
    echo '<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>';
    echo '</head>';
}

/**
 * Flash mesajını SweetAlert2 popup'ı olarak gösteren script üretir.
 * (SweetAlert2 yüklü değilse sessizce atlanır.)
 */
function render_flash_swal(): void
{
    $flash = flash_get();
    if (!$flash) {
        return;
    ...
    $title = $type === 'error' ? 'Hata' : ($type === 'warning' ? 'Uyarı' : 'Başarılı');
    echo '<script>
    (function () {
        function showFlash() {
...
function render_header(string $title, string $active = ''): void
{
    ...
    // Bekleyen kayıt talebi sayısı (yalnızca sistem yöneticisi menüsünde gösterilir)
    $pendingCount = 0;
    if ($role === 'sistem_yoneticisi') {
        try {
        ...
        $nav['kayit_onaylari'] = ['kayit_onaylari.php', 'person_add', 'Kayıt Onayları'];
    }

    render_head($title);
    echo '<body><canvas id="rose-bg" class="pointer-events-none" style="position:fixed; inset:0; z-index:0; display:none; background:transparent;"></canvas><canvas id="ocean-bg" class="pointer-events-none" style="position:fixed; inset:0; z-index:0; display:none; background:transparent;"></canvas><canvas id="dark-bg" class="pointer-events-none" style="position:fixed; inset:0; z-index:0; display:none; background:transparent;"></canvas><canvas id="light-bg" class="pointer-events-none" style="position:fixed; inset:0; z-index:0; display:none; background:transparent;"></canvas><canvas id="corporate-bg" class="pointer-events-none" style="position:fixed; inset:0; z-index:0; display:none; background:transparent;"></canvas><div class="app">';
    ...
    echo '<a class="sb-brand-sub" href="dashboard.php" style="font-size:9px; text-decoration:none; display:block;">Yönetim Paneli</a></div>';
    echo '</div>';
    ...
    foreach ($nav as $key => [$href, $icon, $label]) {
        ...
        $badge = '';
        if ($key === 'kayit_onaylari' && $pendingCount > 0) {
            $badge = ' <span style="margin-left:auto; background:var(--danger); color:#fff; font-size:10px; font-weight:800; min-width:18px; height:18px; padding:0 5px; border-radius:999px; display:inline-flex; align-items:center; justify-content:center;">' . $pendingCount . '</span>';
        }
        echo '<a href="' . $href . '"' . $cls . '><span class="ms">' . svg_icon($icon) . '</span>' . e($label) . $badge . '</a>';
    }
    echo '</nav>';

    echo '<div class="sb-bottom">';
    $cls = 'settings' === $active ? ' class="active"' : '';
    echo '<a href="settings.php"' . $cls . '><span class="ms">' . svg_icon('settings') . '</span>' . e('Ayarlar') . '</a>';
    $cls = 'surum' === $active ? ' class="active"' : '';
    echo '<a href="surum.php"' . $cls . '><span class="ms">' . svg_icon('info') . '</span>' . e('Sürüm') . ' <span style="margin-left:auto; font-size:10px; opacity:.7;">v' . e(APP_VERSION) . '</span></a>';
    echo '</div>';
    ...
    if (app_mode() === 'demo') {
        echo '<div style="display:flex; align-items:center; gap:8px; margin-bottom:16px; padding:9px 16px; border-radius:10px; background:rgba(245,130,32,0.12); border:1px solid rgba(245,130,32,0.35); color:#b54708; font-size:13px; font-weight:600;">'
            . '<span class="ms sm" style="color:#F58220;">' . svg_icon('info') . '</span>'
            . 'DEMO MODU — Bu ekranda örnek (sahte) veriler gösterilmektedir. Gerçek verileriniz etkilenmez.'
            . '</div>';
    }

    // Flash mesajı artık SweetAlert2 popup'ı olarak gösteriliyor
    render_flash_swal();
}

function render_footer(): void
{
    echo '</main><footer class="footer">Stajyer Ahsen Ece Tarafından Yapılmıştır · <a href="surum.php" style="color:inherit; text-decoration:none;">v' . e(APP_VERSION) . '</a></footer></div></div>';

    /* Donma önlemi: Escape ile takılı kalabilecek tüm modal/overlay katmanlarını kapat */
    echo '<script>
    document.addEventListener("keydown", function (ev) {
        if (ev.key === "Escape") {
            document.querySelectorAll(".modal-overlay.open, .user-modal-overlay.open").forEach(function (el) { el.classList.remove("open"); });
            document.querySelectorAll("#viewModal, #formModal, #noteEditModal").forEach(function (el) { if (el) el.classList.add("hidden"); });
        }
    });

    /* Bir işlem (form gönderimi) sonrası sayfa yeniden yüklenince en üste atmasın;
       gönderimden hemen önceki kaydırma konumuna geri dön. */
    (function () {
    ...
    (function () {
        ...
        function drawCard(c) {
            ...
            // İnce "satır" çizgileri — belge hissi
