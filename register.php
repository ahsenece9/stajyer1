...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fullName === '' || $username === '' || $password === '') {
        ...
        if ($stmt->fetch()) {
            ...
            // Kayıt talebi oluştur — durum "pending" (onay bekliyor).
            // Kullanıcı sisteme giriş YAPAMAZ; sistem yöneticisi onaylayıp birim atayana kadar bekler.
            $passHash = password_hash($password, PASSWORD_DEFAULT);
            $stmt = db()->prepare('INSERT INTO users (username, password_hash, full_name, email, role, status) VALUES (?, ?, ?, ?, \'birim_sorumlusu\', \'pending\')');
            $stmt->execute([$username, $passHash, $fullName, $email]);

            // Loglama — oturum açık olmadığı için aday adı ve IP kaydedilir
            log_action('kayit_talebi', 'Yeni kayıt talebi: ' . $fullName . ' (' . $username . ')', $fullName);

            // Otomatik giriş YOK — onay bekleme ekranı gösterilir
            $success = true;
        }
    }
}
...
<head>
    ...
    <title>Samsun Büyükşehir Belediyesi Staj Takip Sistemi - Kayıt Ol</title>
    ...
    <style>
        ...
        #register-form input[type="text"] { padding-left: 3rem !important; padding-right: 1rem !important; }
        #register-form input[type="password"] { padding-left: 3rem !important; padding-right: 3rem !important; }
    </style>
</head>
<body class="min-h-screen flex items-center justify-center overflow-x-hidden py-6">
...
void main() {
    ...
    vec3 bg      = vec3(0.87, 0.90, 0.95);
    vec3 accent1 = vec3(0.35, 0.45, 0.65);
    vec3 accent2 = vec3(0.96, 0.65, 0.35);
    vec3 accent3 = vec3(0.75, 0.78, 0.85);
...
<main class="relative z-10 w-full max-w-5xl mx-4 lg:mx-auto h-[90vh] max-h-[620px] flex shadow-2xl rounded-3xl overflow-hidden bg-white/10 backdrop-blur-sm animate-entrance">
    ...
    <section class="w-full lg:w-1/2 glass-card p-6 md:p-10 flex flex-col overflow-y-auto form-scrollbar">
...
<?php if ($success): ?>
    <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
        <div class="bg-white rounded-2xl p-8 max-w-[440px] w-full text-center shadow-2xl mx-4">
            <span class="material-symbols-outlined text-amber-500 text-[64px] mb-4" style="font-variation-settings: 'FILL' 1;">hourglass_top</span>
            <h3 class="text-2xl font-bold text-gray-900 mb-2">Kayıt Talebiniz Alındı</h3>
            <p class="text-gray-600 mb-6">Hesabınız oluşturuldu ancak <b>sistem yöneticisi onayı</b> bekliyor. Yöneticiniz sizi bir birime atayıp onayladıktan sonra giriş yapabilirsiniz.</p>
            <a href="login.php" class="inline-block px-6 py-3 bg-primary text-white rounded-xl font-semibold text-sm hover:opacity-90 transition-all" style="text-decoration:none;">Giriş Ekranına Dön</a>
        </div>
    </div>
<?php endif; ?>
